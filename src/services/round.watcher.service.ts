...
export class RoundWatcherService {
  ...
  public static getInstance(): RoundWatcherService {
    if (!RoundWatcherService.instance) {
      throw new Error(
        'RoundWatcherService has not been initialized. Call RoundWatcherService.initialize() first.',
      );
    }
    return RoundWatcherService.instance;
  }
  ...
  public start(): void {
    ...
    console.log(
      `RoundWatcherService started — polling every ${this.POLL_INTERVAL / 60000} minutes`,
    );
    ...
    this.intervalId = setInterval(async () => {
      ...
      await Promise.allSettled(rounds.map((round) => this.roundPollingService.poll(round)));
    }, this.POLL_INTERVAL);
  }

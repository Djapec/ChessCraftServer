...
export class RoundPollingService {
  ...
  public async processResponse(
    ...
    const completedPairings = roundData.pairings.filter((p) => !p.live && p.result !== '*').length;
  ...
  public async initializeRound(
    ...
    if (existingRound) {
      ...
      console.log(
        `Round ${key} resumed from DB — ${completedPairings}/${existingRound.pairings.length} pairings completed`,
      );
    } else {
